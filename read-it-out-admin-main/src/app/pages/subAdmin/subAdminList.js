...
export default function SellerDashboard(props) {
  ...
  const EditQuestion = (row) => {
    history.push("/subAdminEdit?id=" + row);
  };
  ...
  const BlockPage = (row) => {
    console.log("/sub_admin/delete/" + row);
    ApiDelete("/sub_admin/delete/" + row)
      .then((res) => {
        if (res.status === 200) {
          handleClose1();
  ...
  const getDate = (s, p, q) => {
    ...
    ApiPost("/sub_admin/get_sub_admin", body)
      .then((res) => {
        setCategory(res.data.data.sub_admin_data);
      ...
      <Row>
        <Col md={12}>
          ...
          <Breadcrumb className="breNav">
            <BreadcrumbItem active>List of All Sub-Admin</BreadcrumbItem>
            {users === 3 ? (
              ""
            ) : (
              <button
                ...
                onClick={() => history.push("/subAdminEdit")}
              >
                <span>Add</span>
              </button>
            )}
