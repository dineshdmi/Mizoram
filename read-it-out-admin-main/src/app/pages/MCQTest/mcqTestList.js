...
export default function SellerDashboard(props) {
  ...
  const BlockPage = (row) => {
    console.log("/question/delete/" + row);
    ApiDelete("/question/delete/" + row)
      .then((res) => {
        if (res.status === 200) {
          handleClose1();
  ...
  const getDate = (s, p, q) => {
    ...
    ApiPost("/question/get_filter_mcq_question", body)
      .then((res) => {
        setCategory(res.data.data.mcqQuestion_data);
              ...
              <div style={spacing}>
                <Col md={6}>
                  <FormGroup>
                    ...
                    >
                      <option value="all">Select Mcq Test List</option>
