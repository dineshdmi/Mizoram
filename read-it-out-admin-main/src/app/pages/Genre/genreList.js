...
export default function SellerDashboard(props) {
  ...
  const BlockPage = (row) => {
    console.log("/genre/delete/" + row);
    ApiDelete("/genre/delete/" + row)
      .then((res) => {
        if (res.status === 200) {
          handleClose1();
  ...
  const getDate = (s, p, q) => {
    ...
    ApiPost("/genre/get_genre", body)
      .then((res) => {
        setCategory(res.data.data.genre_data);
      ...
      <Row>
        <Col md={12}>
          ...
          <Breadcrumb className="breNav">
            <BreadcrumbItem active>List of all genres</BreadcrumbItem>
            <button
