...
export default function SellerDashboard(props) {
  ...
  const [columns, setcolumn] = useState([
    ...
    //     return (
    //       <center>
    //         <div>{row.original.main_category}</div>
    //       </center>
    //     );
    //   },
    // },
  ...
  const BlockPage = (row) => {
    console.log("/category/delete/" + row);
    ApiDelete("/category/delete/" + row)
      .then((res) => {
        if (res.status === 200) {
          handleClose1();
  ...
  const getDate = (s, p, q) => {
    ...
    ApiPost("/category/get_category", body)
      .then((res) => {
        setCategory(res.data.data.category_data);
      ...
      <Row>
        <Col md={12}>
          ...
          <Breadcrumb className="breNav">
            <BreadcrumbItem active>List of all categories</BreadcrumbItem>
            <button
              id="kt_login_signin_submit"
              type="submit"
              className={`btn btn-primary font-weight-bold px-8 py-2`}
              // onClick={() => history.push("/genreEdit")}
              onClick={() => history.push("/categoryedit")}
            >
              <span>Add</span>
            </button>
          </Breadcrumb>
