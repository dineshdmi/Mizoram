...
export default function SellerDashboard(props) {
  ...
  const [columns, setcolumn] = useState([
    ...
    //     return (
    //       <center>
    //         <div>{row.original.main_category[0].name}</div>
    //       </center>
    //     );
    //   },
    // },
    ...
    //     return (
    //       <center>
    //         <div>{row.original.category[0].name}</div>
    //       </center>
    //     );
    //   },
    // },
  ...
  const EditQuestion = (row) => {
    history.push("/subCategoryEdit?id=" + row);
  };
  ...
  const BlockPage = (row) => {
    console.log("/sub_category/delete/" + row);
    ApiDelete("/sub_category/delete/" + row)
      .then((res) => {
        if (res.status === 200) {
          handleClose1();
  ...
  const getDate = (s, p, q) => {
    ...
    ApiPost("/sub_category/get_sub_category", body)
      .then((res) => {
        setCategory(res.data.data.subCategory_data);
      ...
      <Row>
        <Col md={12}>
          ...
          <Breadcrumb className="breNav">
            <BreadcrumbItem active>List of All Sub Categories</BreadcrumbItem>
            <button
              id="kt_login_signin_submit"
              type="submit"
              className={`btn btn-primary font-weight-bold px-8 py-2`}
              onClick={() => history.push("/subCategoryEdit")}
            >
              <span>Add</span>
            </button>
          </Breadcrumb>
